...
(function () {
  ...
  // The engine is a normal script in this page, so it has already run by the
  // time this file does — no eval, which matters because a strict
  // content-security policy may forbid it. Its source is read back out of the
  // same element to start the worker, since a script element's text stays
  // readable after it executes.
  const engineSource = document.getElementById("engine-source").textContent;
  const Engine = window.BaselineVision;
  if (!Engine) throw new Error("Die Engine wurde nicht geladen.");
